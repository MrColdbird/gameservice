...
: m_dwCountMax(count)
    ...
    ,m_Achievements(NULL) 
#elif defined(_PS3)
    ,m_TrophyDetails(NULL),m_TrophyData(NULL),
    m_TrophyHandle(SCE_NP_TROPHY_INVALID_HANDLE),m_TrophyCtx(SCE_NP_TROPHY_INVALID_CONTEXT),
    m_iFreeSpaceAvailable(freespace), m_iTrophySpace(0)
#endif
{
#if defined(_XBOX) || defined(_XENON)
    m_Achievements = new(GSOPType) GS_BYTE[XACHIEVEMENT_SIZE_FULL * m_dwCountMax];
#elif defined(_PS3)
    m_TrophyDetails = new(GSOPType) SceNpTrophyDetails[m_dwCountMax];
    m_TrophyData = new(GSOPType) SceNpTrophyData[m_dwCountMax];
...
void AchievementSrv::ReadAchievements( GS_UINT userIndex, GS_INT startIndex, GS_INT count )
{
    ...
    for (GS_INT i=startIndex; i<startIndex+count; i++)
    {
        ...
        if (ret < 0) {
            ...
            printf("sceNetTrophyGetFlagInfo() failed %x\n", ret);
            return;
        }
    }
...
void AchievementSrv::Write(GS_INT num, GS_INT* ids)
{
    ...
    if( hEventComplete == NULL )
        FatalError( "GS Achievement: Couldn't create event.\n" );
    ...
    XUSER_ACHIEVEMENT* pAchievements = new(GSOPType) XUSER_ACHIEVEMENT[num];
    ...
    if (m_iWantUnlockIDs)
        Free(m_iWantUnlockIDs);

    m_iWantUnlockIDNum = num;
    m_iWantUnlockIDs = new(GSOPType) GS_INT[num];
    for (GS_INT i=0; i<num; i++)
    {
