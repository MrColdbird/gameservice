...
#ifndef GAMESERVICE_USE_CUSTOMIZED_ARRAY
#include <vector>
#endif
...
#ifdef GAMESERVICE_USE_CUSTOMIZED_ARRAY
// Copied from unreal:
enum {INDEX_NONE	= -1         };
...
public:
	...
	GS_VOID ArrayRealloc( GS_DWORD size, GS_DWORD oldNum=0 )
	{
	...
	GS_VOID InsertZeroed( GS_INT Index, GS_INT Count, GS_INT ElementSize )
	{
		Insert( Index, Count, ElementSize );
		memset( (GS_BYTE*)m_Data+Index*ElementSize, 0, Count*ElementSize );
	}

	GS_VOID Insert( GS_INT Index, GS_INT Count, GS_INT ElementSize )
	{
		Assert(Count>=0);
		Assert(m_ArrayNum>=0);
		Assert(m_ArrayMax>=m_ArrayNum);
	...
	GS_VOID Shrink( GS_INT ElementSize )
	{
		Assert(m_ArrayNum>=0);
		Assert(m_ArrayMax>=m_ArrayNum);
	...
	GS_VOID Remove( GS_INT Index, GS_INT Count=1 )
	{
		Assert(Index>=0);
		Assert(Index<=m_ArrayNum);
	...
	GS_VOID Append(const TArray<T>& Source)
	{
...
protected:
	GS_VOID Copy(const TArray<T>& Source)
	{
		if (this != &Source)
		{
...
#endif // GAMESERVICE_USE_CUSTOMIZED_ARRAY
